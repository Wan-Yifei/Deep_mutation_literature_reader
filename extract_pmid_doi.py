...
def get_doi_api(pmids, email, tool_nm="get_doi_api", api_key = "", max_tries = 3, sleep_between_tries = 15, pmids_limit = 200):
    ...
    :param api_key: Personal API key from NCBI. If not set, 3 queries/second are allowed. 10 queries/second with a valid API key;
    ...
    '''
    pmids_split = [pmids[ind * pmids_limit : (ind + 1) * pmids_limit] for ind in range(math.ceil(len(pmids) / pmids_limit))]
    pmids_doi = {}
    ## Config Entrez
    Entrez.email = email
    Entrez.tool = tool_nm
